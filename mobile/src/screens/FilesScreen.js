...
import { families as familiesApi, files as filesApi } from '../utils/api';
import { FILE_TYPES, FILE_TYPE_LABELS } from '../utils/constants';
import { canAccessFamilyFeature, formatRelativeDate, guessMimeType } from '../utils/helpers';
  ...
  const uploadImage = async () => {
    ...
    if (!result.canceled && result.assets[0]) {
      try {
        ...
        const attachmentName = result.assets[0].fileName || 'file.jpg';
        const mimeType = guessMimeType(attachmentName, result.assets[0].mimeType);
        ...
        formData.append('file_type', mimeType.startsWith('image/') ? 'image' : 'document');
        if (selectedFamily) formData.append('familyId', selectedFamily);
