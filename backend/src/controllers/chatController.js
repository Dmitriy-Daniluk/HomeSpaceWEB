...
exports.getChatFamilies = async (req, res, next) => {
  try {
    const [families] = await pool.query(
      `SELECT
       ...
       FROM families f
       JOIN family_members fm ON fm.family_id = f.id AND fm.user_id = ?
       LEFT JOIN family_members members ON members.family_id = f.id
       LEFT JOIN chat_messages cm ON cm.family_id = f.id
       GROUP BY f.id, f.name, f.description, fm.role
       ORDER BY COALESCE(MAX(cm.created_at), f.created_at) DESC`,
      [req.user.id]
    );

    res.json({ message: 'Chat families retrieved', data: families });
  } catch (error) {
    next(error);
  }
};
