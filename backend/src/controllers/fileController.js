...
const serializeAttachment = (attachment) => {
  const fileName = normalizeUploadFileName(attachment?.file_name);
...
exports.uploadFile = async (req, res, next) => {
  try {
    ...
    const fileName = normalizeUploadFileName(
      req.body.fileName || req.body.file_name || req.file.originalname
    );
...
exports.getFiles = async (req, res, next) => {
  try {
    ...
    res.json({ message: 'Files retrieved', data: files.map(serializeAttachment) });
  } catch (error) {
    next(error);
  }
};
...
exports.renameFile = async (req, res, next) => {
  try {
    const { id } = req.params;
    const fileName = normalizeUploadFileName(req.body.fileName || req.body.file_name || '');
    ...
    res.json({ message: 'File renamed', data: serializeAttachment(updated[0]) });
  } catch (error) {
    next(error);
  }
};
