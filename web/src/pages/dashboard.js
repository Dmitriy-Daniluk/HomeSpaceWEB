...
export default function DashboardPage() {
  ...
  const fetchFamilies = useCallback(async () => {
    try {
      ...
      setFilters((prev) => {
        const preferredFamilyId = [queryFamilyId, prev.familyId]
          .find((candidate) => candidate && data.some((family) => String(family.id) === String(candidate)))
          || (data[0] ? String(data[0].id) : '');

        return { ...prev, familyId: preferredFamilyId };
      });
  ...
  const createTask = async (e) => {
    ...
    try {
      ...
      if (taskAttachment && createdTask?.id) {
        ...
        formData.append('file_name', taskAttachment.name || 'attachment');
