...
export default function BudgetPage() {
  ...
  useEffect(() => {
    if (mode === 'family') {
      if (families.length > 0 && families[0]?.id) {
        fetchData(families[0].id);
      }
    } else {
      fetchData(null);
    }
  }, [mode, period, customStart, customEnd, typeFilter, families]);
  ...
  const addTransaction = async (e) => {
    ...
    setAdding(true);
    try {
      const params = mode === 'family' ? { familyId: families[0]?.id } : {};
      await api.post('/budget', { ...newTx, amount: Number(newTx.amount) }, { params });
      closeModal();
      fetchData(mode === 'family' ? families[0]?.id : null);
    } catch (err) { console.error(err); }
  ...
  const exportPDF = () => {
    ...
    doc.text(modeTitle, 14, 22);
    doc.setFontSize(11);
    doc.text(`Доход: ${stats.income}₽ | Расход: ${stats.expense}₽ | Баланс: ${stats.balance}₽`, 14, 32);
    autoTable(doc, {
      startY: 40,
      head: [['Тип', 'Категория', 'Сумма', 'Описание', 'Дата']],
  ...
  const familyHasSubscription = families[0]?.has_subscription;
            ...
            {loading ? <Loading /> : transactions.length === 0 ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    {transactions.map((t) => (
                      <TransactionRow key={t.id} transaction={t} onDelete={deleteTransaction} onEdit={openEditModal} />
                    ))}
                  </tbody>
                </table>
              </div>
