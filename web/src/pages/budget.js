...
export default function BudgetPage() {
  ...
  const fetchFamilies = useCallback(async () => {
    try {
      ...
      const currentFamilyId = String(selectedFamilyId || '');
      const preferredFamilyId = [queryFamilyId, currentFamilyId]
  ...
  }, [router.query.familyId, selectedFamilyId]);

  const fetchData = async (familyId, { background = false } = {}) => {
  ...
  const refreshPageData = () => {
    ...
    if (mode === 'family') {
      if (selectedFamilyId) {
        fetchData(selectedFamilyId, { background: true });
      }
      return;
    }

    fetchData(null, { background: true });
  };

  useEffect(() => {
    if (router.isReady) fetchFamilies();
  }, [router.isReady, fetchFamilies]);
  ...
  useAutoRefresh(refreshPageData, { enabled: router.isReady });
