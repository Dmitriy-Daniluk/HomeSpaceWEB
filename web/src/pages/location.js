...
export default function LocationPage() {
  ...
  const fetchFamilies = async () => {
    try {
      ...
      setSelectedFamilyId((current) => {
        const preferredFamilyId = [current]
  ...
  const fetchMembers = async ({ background = false } = {}) => {
    ...
    try {
      ...
      const familyRes = await api.get(`/families/${selectedFamilyId}`);
      ...
      for (const m of familyMembers) {
        try {
          ...
          allMembers.push({ ...m, familyName: family.name, location: locRes.data.data || null });
        } catch {
          allMembers.push({ ...m, familyName: family.name, location: null });
        }
      }
