...
import { MapPin, Plus, Trash2, Navigation, Users, Circle, Crosshair, Clock } from 'lucide-react';
...
export default function LocationPage() {
        ...
        if (f.members) {
          for (const m of f.members) {
            try {
              ...
              allMembers.push({ ...m, familyName: f.name, location: locRes.data });
            } catch {
              allMembers.push({ ...m, familyName: f.name, location: null });
            }
          }
        }
      }
  ...
  const fetchGeofences = async () => {
    try {
      if (families[0]?.id) {
        const res = await api.get(`/location/geofences/${families[0].id}`);
  ...
  const createGeofence = async (e) => {
    ...
    setSaving(true);
    try {
      await api.post(`/location/geofences/${families[0].id}`, {
        ...geofenceForm,
  ...
  return (
    <>
      ...
      <div className="space-y-6 animate-fade-in">
        ...
        {/* Map Placeholder */}
        <Card className="h-64 flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 border-dashed">
          <div className="text-center">
            <MapPin className="w-12 h-12 text-indigo-400 dark:text-indigo-500 mx-auto mb-3" />
            <p className="text-gray-500 dark:text-gray-400 font-medium">Карта</p>
            <p className="text-sm text-gray-400 dark:text-gray-500">Интеграция с картой скоро будет доступна</p>
          </div>
        </Card>
              ...
              {members.map((m) => (
                <div key={m.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-gray-900 dark:text-white text-sm">{m.fullName}</p>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400">
                        {g.latitude?.toFixed(4)}, {g.longitude?.toFixed(4)} • {g.radius}м
                      </p>
                    </div>
                  </div>
                  <button
