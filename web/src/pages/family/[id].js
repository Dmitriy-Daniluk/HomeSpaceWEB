...
import { Users, Mail, Copy, UserPlus, Settings, Trash2, Edit2, CheckSquare, Wallet, ArrowLeft, Target, Share2 } from 'lucide-react';
...
export default function FamilyPage() {
  ...
  const fetchFamily = async () => {
    ...
    try {
      ...
      setEditGoal(res.data.data.savings_goal || '');
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
              ...
              {family.members?.map((member) => (
                <div key={member.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl group">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge variant={member.role === 'owner' ? 'primary' : member.role === 'parent' ? 'warning' : 'default'} size="sm">
                      {member.role === 'owner' ? 'Владелец' : member.role === 'parent' ? 'Родитель' : 'Участник'}
                    </Badge>
